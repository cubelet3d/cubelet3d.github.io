...
const oracleSounds = {
	...
	'harp': new Audio('oracle/sfx/harp.mp3'),
	'knock': new Audio('oracle/sfx/knock.mp3')
}
...
function initOracle() {
    const greeceTime = new Date(new Date().toLocaleString("en-US", {
    ...
    try {
        ...
        cardEls.forEach((wrap, i) => {
            ...
            wrap.onclick = () => {
				...
				readingEl.classList.remove("reveal-text");
				void readingEl.offsetWidth;
				readingEl.classList.add("reveal-text");
				
				oracleCardSound();
				oracleTyping = true; 

				typeText(readingEl, parsed[label], () => {
					oracleTyping = false;
				});
...
function oracleCardSound() {
    const randomNumber = Math.floor(Math.random() * 4) + 1;
    const sound = oracleSounds[`card_place_0${randomNumber}`];
    sound.play();
}

function typeText(el, text, done, speed = 16) {
    el.textContent = "";
    let i = 0;
